...
def nir_extract(filename, outname, x1=0, x2=0, y1=0, y2=0,
   ...
   """ Set up NIRSPEC detector characteristics. Need to change this to fit MOSFIRE detector. """
   gain = 4.     # Value in e-/ADU
   rdnoise = 25. # Value in e-

   """ set apmin and apmax of object KH.10 """
   apmin = 28
   apmax = 60
   """ Read the 2d spectrum """
   print ""
   d,w,v = read_mosfire_spec(filename,x1,x2,y1,y2,informat=informat)
   if informat=='old':
      s = v
   else:
      s = numpy.sqrt(v)
