...
def plot_spectrum_array(x, flux, var=None, xlabel="Wavelength (Angstroms)",
                        ...
                        docolor=True, rmsoffset=0, rmsls=None, fontsize=12,xmin=0,xmax=0):
...
def trace_spectrum(data,mu0,sig0,dispaxis="x",stepsize=25,muorder=3,
   ...
   if(do_subplot):
      plt.subplot(223)
   # else:                # <----- Not sure why this makes a figure of nothing
   #    plt.figure(3)
   #    plt.clf()
...
def extract_spectrum(data,mupoly,sigpoly,dispaxis="x",apmin=-4.,apmax=4.,
                     ...
                     do_plot=True, do_subplot=False, outfile=None,xmin=0,xmax=0):
   """
   ...
   # Set the wavelength axis. Still in units of pixels though.
   ...
   if do_plot:
      ...
      if(do_subplot):
         plt.subplot(224) # lower right corner?
      else:
         plt.figure(4)
         plt.clf()
      plot_spectrum_array(pix,amp,title='Extracted spectrum (pixels)',
                          xlabel='Pixel number in the dispersion direction',xmin=xmin,xmax=xmax)
   plt.show()
